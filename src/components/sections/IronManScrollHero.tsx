...
export default function IronManScrollHero() {
  ...
  const engageBtnRef = useRef<HTMLDivElement | null>(null);
  ...
  // 2. Draw frame on canvas with aspect ratio cover/contain
  ...
  // 4. Scroll listener: high-performance scrub calculation
  useEffect(() => {
    const handleScroll = () => {
      ...
      requestAnimationFrame(() => {
        ...
        if (heroTextRef.current) {
          ...
          heroTextRef.current.style.pointerEvents = opacity < 0.05 ? "none" : "auto";
        }

        // Protocol text fade in/out
        if (bigLeftTextRef.current) {
          let op = 0;
          if (progress >= 0.1 && progress <= 0.45) {
            op = Math.min(1, Math.max(0, (progress - 0.1) / 0.08));
          } else if (progress > 0.45) {
            op = Math.max(0, 1 - (progress - 0.45) / 0.08);
          }
          bigLeftTextRef.current.style.opacity = String(op);
          bigLeftTextRef.current.style.transform = `translateY(${(1 - op) * 14}px)`;
        }

        // Bottom progress bar scale
        ...
        if (seqReadoutRef.current) {
          const currentSeq = Math.min(FRAME_COUNT, frameIndex + 1);
          seqReadoutRef.current.textContent = `SEQ ${String(currentSeq).padStart(3, "0")} / ${FRAME_COUNT}`;
        }
        ...
        // Engage button near completion
        if (engageBtnRef.current) {
          const btnOp = Math.min(1, Math.max(0, (progress - 0.85) / 0.08));
          engageBtnRef.current.style.opacity = String(btnOp);
          engageBtnRef.current.style.transform = `translateY(${(1 - btnOp) * 12}px)`;
          engageBtnRef.current.style.pointerEvents = btnOp > 0.3 ? "auto" : "none";
        }

        // Quote cards visibility
  ...
  const handleSkipToNext = () => {
    const nextEl = document.getElementById("content");
    if (nextEl) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* 4 Corner HUD Frames */}
        <div className="pointer-events-none absolute left-6 top-24 text-accent md:left-10 md:top-28 z-20">
          <HudFrame corner="tl" size={26} />
        </div>
        <div className="pointer-events-none absolute right-6 top-24 text-accent md:right-10 md:top-28 z-20">
          <HudFrame corner="tr" size={26} />
        </div>
        <div className="pointer-events-none absolute bottom-14 left-6 text-accent md:bottom-16 md:left-10 z-20">
          <HudFrame corner="bl" size={26} />
        </div>
        <div className="pointer-events-none absolute bottom-14 right-6 text-accent md:bottom-16 md:right-10 z-20">
          <HudFrame corner="br" size={26} />
        </div>

        {/* Hero Initial Announcement */}
        <div
          ref={heroTextRef}
        ...
        >
          ...
          <p className="max-w-[42ch] font-sans text-sm leading-relaxed text-zinc-400 md:text-base">
            ...
            system diagnostic &mdash; J.A.R.V.I.S. is holding on the line.
          </p>
        </div>

        {/* Protocol Middle Stage */}
        <div
          ref={bigLeftTextRef}
        ...
        <div className="pointer-events-none absolute inset-x-0 bottom-0 z-10">
          ...
          <div className="mx-6 flex items-center justify-between pb-4 font-mono text-[10px] uppercase tracking-[0.28em] text-zinc-500 md:mx-10">
            <span ref={seqReadoutRef}>SEQ 001 / 169</span>
              ...
              <figure
                ...
                className={`card-surface pointer-events-auto p-5 transition-all duration-400 ease-out ${
                  visible
                    ...
                    : "translate-y-4 opacity-0 h-0 p-0 overflow-hidden border-none"
                }`}
              >
        ...
        {/* Engage / Jump Button near end of scroll */}
        <div
          ref={engageBtnRef}
          className="pointer-events-none absolute bottom-24 right-6 z-20 hidden flex-col items-end gap-3 md:flex md:bottom-28 md:right-12 opacity-0 transition-[opacity,transform] duration-150"
        >
          ...
          <button
            ...
            className="pointer-events-auto inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/[0.08] px-5 py-2.5 font-mono text-[11px] font-medium uppercase tracking-[0.22em] text-foreground backdrop-blur-md transition-all duration-200 hover:bg-white/[0.14] hover:text-accent active:translate-y-[1px] cursor-pointer"
          >
            Open Portfolio Systems &darr;
          </button>
        </div>

        {/* Preloader / Booting Overlay */}
        {!loaded && (
